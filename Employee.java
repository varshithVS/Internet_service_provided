...
ResultSet rs;
    ...
    void showDate(){
        ...
        public void Update(){
        try{
            pst = conn.prepareStatement("UPDATE `employee` SET LeaveDate=?, name='"+jTextField2.getText()+"', contact='"+jTextField3.getText()+"', joindate='"+jTextField4.getText()+"',address='"+jTextField5.getText()+"'  WHERE ID=?");
        ...
        public void Search(){
        try{
        String sql = "SELECT `ID`, `Name`, `Contact`, `JoinDate`, `Address`, `LeaveDate` from employee WHERE id=?";
        pst=conn.prepareStatement(sql);
